...
void setMotorPWM(uint8_t rxFrame[])
{
  ...
  if (rxFrame[0] < 100 && rxFrame[0] < 100 && rxFrame[0] < 100 && rxFrame[0] < 100)
    return;
  uint32_t period = __HAL_TIM_GET_AUTORELOAD(&htim1);

    //TODO: check if pwm is calculated correctly
  __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, rxFrame[0]/100.f * (period + 1));//tmp asf
  __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, rxFrame[1]/100.f * (period + 1));//tmp asf
  __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, rxFrame[2]/100.f * (period + 1));//tmp asf
  __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, rxFrame[3]/100.f * (period + 1));//tmp asf
}
...
{
  HAL_GPIO_WritePin(LED_GPIO_Port, LED_GPIO_PIN,(rxFrame[4] > 0)); //TODO: change after unbloatings
