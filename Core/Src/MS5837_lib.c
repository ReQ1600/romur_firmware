...
{   // Based on Blue Robotics MS5837 Arduino Library
	...
	SENS = (int64_t)(sensor -> prom_coefficients[1]) * 65536 + ((int64_t)(sensor -> prom_coefficients[3])*dT) / 128;
	OFF  = (int64_t)(sensor -> prom_coefficients[2]) * 131072 + ((int64_t)(sensor -> prom_coefficients[4])*dT) / 64;
	sensor -> pressure_raw = (sensor -> pressure_D1 * SENS / (2097152) - OFF) / (32768);
	...
	if((sensor -> temperature_raw / 100) < 20) 			//--- Low temp
	{
		...
		OFFi 	= (31 * (sensor -> temperature_raw - 2000) * (sensor -> temperature_raw - 2000)) / 8;
		SENSi 	= (63 * (sensor -> temperature_raw - 2000) * (sensor -> temperature_raw - 2000)) / 32;
	}
	...
	sensor -> pressure_raw 	  		= (((sensor -> pressure_D1 * SENS2) / 2097152 - OFF2) / 32768);

	sensor -> temperature_celsius 	= sensor -> temperature_raw / 100.0;
	sensor -> pressure_mbar   		= sensor -> pressure_raw / 100.0;
